...
import { FiArrowRight, FiCheckCircle, FiFileText, FiGrid, FiHeart, FiLayers, FiMic, FiSearch, FiShield, FiTarget, FiZap, FiPlus, FiChevronRight } from "react-icons/fi";
import "../css/dashboard.css";

const ResumePreview = React.memo(({ closePreviewModal }) => <ScrollableModal onClose={closePreviewModal} header={<Heading>Resume Preview</Heading>}><ClassicalLayout1 /></ScrollableModal>);
    ...
    <div className="workspace-shell">
      ...
      <main className="workspace-main">
        ...
        <section className="welcome-row">
          <div><span className="eyebrow">YOUR HEALTHCARE CAREER WORKSPACE</span><h1>Good to see you, {firstName}.</h1><p>Everything you need to build, improve and practise for your next healthcare opportunity.</p></div>
          <Link className="primary-cta" to="/ats-checker"><FiPlus /> Improve my CV</Link>
        </section>

        <section className="workspace-grid">
          <article className="readiness-card">
            <div className="readiness-head"><div><span className="eyebrow">CAREER READINESS</span><h2>Keep your application moving</h2><p>Start with your CV, then use AI to prepare for the role.</p></div><div className="readiness-ring"><strong>{resumes.length ? "CV" : "0"}</strong><span>{resumes.length ? "created" : "start"}</span></div></div>
            <div className="readiness-steps"><Link to="/ats-checker"><span className="step-dot done"><FiCheckCircle /></span><div><strong>CV foundation</strong><small>{resumes.length ? "Your workspace has a saved CV" : "Create your first healthcare CV"}</small></div><FiChevronRight /></Link><Link to="/ats-checker"><span className="step-dot"><FiSearch /></span><div><strong>ATS optimisation</strong><small>Check keywords, structure and improvement opportunities</small></div><FiChevronRight /></Link><Link to="/interview-coach"><span className="step-dot"><FiMic /></span><div><strong>Interview practice</strong><small>Practise with an AI healthcare interviewer</small></div><FiChevronRight /></Link></div>
          </article>
        ...
        <section className="tool-section"><div className="section-heading"><div><span className="eyebrow">CAREER TOOLKIT</span><h2>Build, optimise, practise</h2></div><Link to="/healthcare-guide">Explore guidance <FiArrowRight /></Link></div><div className="tool-grid">
          <Link to="/ats-checker" className="tool-card featured"><div className="tool-icon"><FiSearch /></div><div><span>CV INTELLIGENCE</span><h3>ATS Checker</h3><p>Upload your CV and see what to improve before applying.</p></div><FiArrowRight /></Link>
          <Link to="/career-copilot" className="tool-card"><div className="tool-icon"><FiTarget /></div><div><span>AI CAREER COPILOT</span><h3>Target a vacancy</h3><p>Compare your CV with a job and close the gaps.</p></div><FiArrowRight /></Link>
          <Link to="/interview-coach" className="tool-card"><div className="tool-icon"><FiMic /></div><div><span>AI INTERVIEW</span><h3>Mock interview</h3><p>Answer realistic healthcare questions and get feedback.</p></div><FiArrowRight /></Link>
          <Link to="/practice-questions" className="tool-card"><div className="tool-icon"><FiCheckCircle /></div><div><span>PRACTICE LAB</span><h3>Practice questions</h3><p>Build confidence one healthcare question at a time.</p></div><FiArrowRight /></Link>
        </div></section>
