...
import { FiArrowRight, FiCheckCircle, FiFileText, FiSearch, FiTarget, FiZap, FiAlertTriangle, FiCopy } from "react-icons/fi";
...
export default function CareerCopilot() {
  ...
  return <main className="copilot-page">
    ...
    <section className="copilot-shell">
      <nav className="copilot-tabs">{tools.map(({ id, icon: Icon, title, text }) => <button key={id} className={tool === id ? "active" : ""} onClick={() => { setTool(id); setResult(null); }}><Icon/><span><strong>{title}</strong><small>{text}</small></span></button>)}</nav>
