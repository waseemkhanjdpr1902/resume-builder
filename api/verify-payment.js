...
export default async function handler(request, response) {
  ...
  const { razorpay_order_id: orderId, razorpay_payment_id: paymentId, razorpay_signature: signature, planId } = request.body || {};
  if (!orderId || !paymentId || !signature || !(planId in accessSeconds)) return response.status(400).json({ error: "Incomplete payment verification data" });
  ...
  const payload = encode({ planId, paymentId, issuedAt: now, expiresAt: accessSeconds[planId] ? now + accessSeconds[planId] : null });
