...
export default async function handler(request,response){
  ...
  if(!type){
    ...
    if(dbUrl&&serviceKey){
      ...
      if(entitlement&&(!entitlement.expires_at||new Date(entitlement.expires_at)>new Date()))return response.status(200).json({active:true,planId:entitlement.plan_id,expiresAt:entitlement.expires_at,source:"account"});
    }
    ...
    }catch{return response.status(200).json({active:false});}
  }
