...
export default async function handler(request,response){
 ...
 if(Number(request.headers["content-length"]||0)>1000000)return response.status(413).json({error:"Webhook is too large"});
 ...
 if(!payment?.id||!Object.prototype.hasOwnProperty.call(durations,planId))return response.status(400).json({error:"Incomplete payment event"});
